...
function StateManagement({ ...pageProps }) {
  ...
  const handleSaveTodos = async (e: any) => {
    ...
    try {
      ...
      alert("Successfully added pocheng");
    } catch (error) {
  ...
  const handleDeleteConfirmTodo = async (confirmation: any, id: any) => {
    if (confirmation) {
      try {
        const docRef = doc(db, "pocheng", id); // Delete by document ID
  ...
  const handleEditTodo = (id: any) => {
    const index =
      stateManagement?.todos.findIndex((todo) => todo.id == id) || 0; //findIndex to be edit
    ...
    if (!isEmpty(stateManagement?.todos)) {
      todos = _.sortBy(stateManagement?.todos, ["date"]).map((v: any, k) => {
        ...
        return (
          <tr
            key={k}
            className={` bg-white hover:bg-gray-200 odd:bg-white even:bg-gray-100 ${
              v.status ? "line-through" : ""
            }`}
          >
            <td className="py-5 px-3 align-top w-1/4">
              <p>{v.title}</p>
            </td>
            <td className="flex flex-col py-5 px-3 align-top w-full">
                  ...
                  <table className="w-full table-auto border-collapse">
                    <thead className="text-left">
                      <tr>
                        ...
                        <th className="text-lg text-gray-900 px-3 py-3 border-b-2 border-gray-200 bg-gray-100 text-xs font-semibold text-gray-600 uppercase tracking-wider text-center w-fit">
                          <div className="flex justify-center items-center gap-2">
                            <div className="w-6">
                              <CogIcon />
                            </div>
